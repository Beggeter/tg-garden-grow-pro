...
import { getActiveBoosters } from '../db/localDb';
...
export const calcHarvest = (plantedAt: Date, type: string): number => {
  const rule = PLANT_RULES[type];
  if (!rule) return 0;
  const elapsed = (Date.now() - plantedAt.getTime()) / 60000; // в минутах
  const multiplier = getBoostMultiplier();
  return Math.floor(elapsed / rule.growMinutes) * rule.yieldPerHour * multiplier;
};

function getBoostMultiplier(): number {
  const boosters = getActiveBoosters();
  if (boosters.includes('booster_3x')) return 3;
  if (boosters.includes('booster_2x')) return 2;
  return 1;
}
